...
export default class Parser {
  ...
  require(...expected: TokenType[]): Token {
    ...
    if (!token) {
      throw new Error(
        `On position ${this.pos} ${expected[0].name} was expected`
      );
    }
    return token;
  }
  ...
  parseVariableOrNumber(): ExpressionNode {
    ...
    throw new Error(
      `On position ${this.pos} a number or a variable were expected`
    );
  }
  ...
  parsePrint(): ExpressionNode {
    ...
    throw new Error(`Operator LOG was expected on position ${this.pos}`);
  }
  ...
  parseExpression(): ExpressionNode {
    ...
    throw new Error(
      `After a variable an operator ASSIGN is expected on position ${this.pos}`
    );
  }
  ...
  run(node: ExpressionNode): any {
    ...
    if (node instanceof VariableNode) {
      if (this.scope[node.variable.text]) {
        ...
        throw new Error(
          `Variable with this name ${[node.variable.text]} is not found`
        );
      }
    }
